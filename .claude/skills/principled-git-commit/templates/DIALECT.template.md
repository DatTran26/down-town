...
## 1. Language Default & Domain Proper Nouns

**Default language: English** — summary AND body both. The whitelist in §1.1 is the **only** native-language allowed. Anything outside the whitelist must be translated.

This explicit inverse rule blocks the *partial native-language drift* anti-pattern (universal SKILL.md §11): LLMs reading recent `git log` see whitelisted native terms and over-generalize to "native prose is OK", which compounds across commits. Whitelist-alone is not enough — the default must be stated.

Concrete drift examples (replace with patterns relevant to your locale):
- ❌ `책임 분리` (ko) → ✅ `responsibility separation`
- ❌ `責任分離` (ja) → ✅ `responsibility separation`
- ❌ `separación de responsabilidades` (es) → ✅ `responsibility separation`
- ❌ `진행중 발송` (ko) → ✅ `in-progress send`
- ✅ `친구톡 발송`, `LGPD 컴플라이언스` — whitelist hits, keep as-is

File paths / function names / shared symbols stay in code form (English / camelCase / kebab-case) regardless of locale.

### 1.1 Whitelist (proper nouns — keep in original script)

Terms that should NOT be translated. Keep in original script in summary and body.
